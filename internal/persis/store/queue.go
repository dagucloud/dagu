...
// EnsureEnqueued publishes one queue item for an enqueue key.
func (s *QueueStore) EnsureEnqueued(
	ctx context.Context,
	name string,
	...
	if key == "" {
		return fmt.Errorf("queue store: enqueue key is required")
	}

	itemID, queuedAt, err := ensuredQueueItemID(priority, key)
	if err != nil {
		return err
	}
	payload := queueItemPayload{
	...
	return s.withQueueLock(ctx, name, func() error {
		...
		if errors.Is(err, persis.ErrConflict) {
			existing, getErr := s.col.Get(ctx, recordID)
			if getErr != nil {
				return getErr
			}
			var existingPayload queueItemPayload
			if decodeErr := persis.Decode(existing, &existingPayload); decodeErr != nil {
				return fmt.Errorf("queue store: decode existing item: %w", decodeErr)
			}
			if existingPayload.EnqueueKey != key || existingPayload.DAGRun != dagRun {
				return fmt.Errorf("queue store: enqueue key conflicts with an existing item")
			}
			err = nil
		}
		if err != nil {
