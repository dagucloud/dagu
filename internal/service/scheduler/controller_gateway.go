...
func (g *controllerChildRunGateway) EnsureEnqueued(ctx context.Context, request controller.ChildRunRequest) error {
	...
	attempt, err := g.dagRunStore.FindAttempt(ctx, dagRun)
	switch {
	case err == nil:
...
func (g *controllerChildRunGateway) republishQueuedAttempt(
	...
	if err := g.queueStore.Enqueue(ctx, dag.ProcGroup(), exec.QueuePriorityLow, dagRun); err != nil {
...
func (g *controllerChildRunGateway) enqueueNewAttempt(ctx context.Context, request controller.ChildRunRequest) error {
	...
	_, err = intake.EnqueueRun(ctx, intake.QueueRequest{
		DAGRunStore:     g.dagRunStore,
		QueueStore:      g.queueStore,
		DAG:             resolved,
...
func (g *controllerChildRunGateway) Observe(
	...
	if outputs != nil {
		for key, value := range outputs.Outputs {
			observation.Outputs[key] = value
		}
	}
...
func (g *controllerChildRunGateway) Stop(ctx context.Context, request controller.ChildRunRequest) error {
	...
	if g.autoRetryPending(status) {
		if err := exec.CancelFailedAutoRetryPendingRun(ctx, g.dagRunStore, status); err != nil {
	...
	if status.Status == core.Queued {
		...
		if err := g.procStore.Lock(ctx, queueName); err != nil {
			return fmt.Errorf("lock Controller child queue %q: %w", queueName, err)
		}
		defer g.procStore.Unlock(ctx, queueName)

		if err := exec.AbortQueuedDAGRun(ctx, g.dagRunStore, dagRun); err != nil {
			return fmt.Errorf("abort queued Controller child DAG run: %w", err)
		}
		if _, err := g.queueStore.DequeueByDAGRunID(ctx, queueName, dagRun); err != nil && !errors.Is(err, exec.ErrQueueItemNotFound) {
...
func (g *controllerChildRunGateway) autoRetryPending(status *exec.DAGRunStatus) bool {
	if g == nil || g.scheduler == nil || g.scheduler.config == nil {
		return false
	}
	if g.scheduler.retryScanner != nil {
		return g.scheduler.retryScanner.autoRetryPending(status)
	}
	return (&RetryScanner{
		retryWindow: g.scheduler.config.Scheduler.RetryFailureWindow,
		clock:       g.scheduler.clock,
	}).autoRetryPending(status)
}
