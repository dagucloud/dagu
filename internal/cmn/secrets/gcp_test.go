...
func TestGCPSecretManagerResolverValidate(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "ShortID", ref: ir.SecretRef{Key: "database-password"}},
		{name: "GlobalSecret", ref: ir.SecretRef{Key: "projects/project-a/secrets/database-password"}},
		{name: "GlobalVersion", ref: ir.SecretRef{Key: "projects/project-a/secrets/database-password/versions/5"}},
		{name: "RegionalSecret", ref: ir.SecretRef{Key: "projects/project-a/locations/us-central1/secrets/database-password"}},
		{name: "RegionalVersion", ref: ir.SecretRef{Key: "projects/project-a/locations/us-central1/secrets/database-password/versions/latest"}},
		{name: "Empty", ref: ir.SecretRef{}, wantErr: "key"},
		{name: "ShortIDWithSlash", ref: ir.SecretRef{Key: "team/database-password"}, wantErr: "must not contain slashes"},
		{name: "MalformedResource", ref: ir.SecretRef{Key: "projects/project-a/locations/us-central1/secrets"}, wantErr: "invalid"},
		{name: "VersionConflict", ref: ir.SecretRef{Key: "projects/project-a/secrets/database-password/versions/5", Options: map[string]string{"version": "6"}}, wantErr: "conflicts"},
		{name: "ResourceWhitespaceOptions", ref: ir.SecretRef{Key: "projects/project-a/secrets/database-password/versions/5", Options: map[string]string{"project_id": " ", "location": " ", "version": " "}}},
		{name: "ProjectConflict", ref: ir.SecretRef{Key: "projects/project-a/secrets/database-password", Options: map[string]string{"project_id": "project-b"}}, wantErr: "cannot be used"},
		{name: "InvalidLocation", ref: ir.SecretRef{Key: "database-password", Options: map[string]string{"location": "evil.example.com:443"}}, wantErr: "invalid characters"},
...
func TestGCPSecretManagerResolverResolve(t *testing.T) {
	...
	got, err := resolver.Resolve(ctx, ir.SecretRef{Key: "database-password", Options: map[string]string{"field": "token"}})
	require.NoError(t, err)
	...
	got, err = resolver.Resolve(context.Background(), ir.SecretRef{Key: "projects/full-project/secrets/name/versions/7"})
	require.NoError(t, err)
	assert.Equal(t, string(data), got)

	got, err = resolver.Resolve(context.Background(), ir.SecretRef{
...
func TestGCPSecretManagerResolverErrors(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := resolver.Resolve(context.Background(), ir.SecretRef{Key: "projects/project-a/secrets/name/versions/latest"})
			require.ErrorContains(t, err, tc.wantErr)
		})
	}
}
...
func TestGCPSecretManagerResolverClose(t *testing.T) {
	...
	for _, location := range []string{"", "us-central1"} {
		_, err := resolver.Resolve(context.Background(), ir.SecretRef{
