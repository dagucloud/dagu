...
func TestKubernetesResolver_Resolve(t *testing.T) {
	...
	value, err := resolver.Resolve(ctx, ir.SecretRef{
		Name:     "DB_PASSWORD",
		Provider: "kubernetes",
		Key:      "app-secrets/db-password",
	})
...
func TestKubernetesResolver_ResolveWithOptions(t *testing.T) {
	...
	value, err := resolver.Resolve(context.Background(), ir.SecretRef{
		Name:     "API_TOKEN",
		Provider: "kubernetes",
		Key:      "api-token",
...
func TestKubernetesResolver_ResolveClientSettings(t *testing.T) {
	...
	settings := resolver.resolveClientSettings(ctx, ir.SecretRef{
		Options: map[string]string{
