...
func validateHumanTaskFormShape(form map[string]any) error {
	for name := range form {
		if _, ok := humanTaskFormRootFields[name]; !ok {
			return fmt.Errorf("unsupported form schema field %q", name)
		}
	}
	if formType, ok := form["type"].(string); !ok || formType != "object" {
		return fmt.Errorf("form type must be object")
	}
	...
	if !ok {
		return fmt.Errorf("form properties must be an object")
	}
	if value, ok := form["additionalProperties"]; ok {
		if _, ok := value.(bool); !ok {
			return fmt.Errorf("form additionalProperties must be a boolean")
		}
	}
	...
	for name, rawProperty := range properties {
		...
		if options, exists := property["oneOf"]; exists {
			...
			for idx, rawOption := range items {
				...
				if err := validateHumanTaskStringMetadata(option, fmt.Sprintf("form property %q oneOf[%d]", name, idx)); err != nil {
					return err
				}
				if optionType, exists := option["type"]; exists {
					if err := validateHumanTaskScalarType(optionType, fmt.Sprintf("form property %q oneOf[%d] type", name, idx)); err != nil {
						return err
					}
				}
