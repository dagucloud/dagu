...
func buildActionsPrompt(env EnvironmentInfo, access *auth.WorkspaceAccess) string {
	...
	b.WriteString("- For DAG-run artifacts in authored DAGs, prefer `stdout.artifact`/`stderr.artifact` for command streams and built-in `artifact.write`, `artifact.read`, and `artifact.list` for explicit artifact operations; avoid `run:` commands that write to `DAG_RUN_ARTIFACTS_DIR`.\n")
