...
// Package orcarouter provides an LLM provider implementation for OrcaRouter's API.
// OrcaRouter is a unified gateway that routes requests to various LLM providers
// (Anthropic, OpenAI, etc.) behind a single OpenAI-compatible Chat Completions endpoint.
package orcarouter
...
func init() {
	llm.RegisterProvider(llm.ProviderOrcaRouter, New)
}

// Provider implements the llm.Provider interface for OrcaRouter.
...
func (p *Provider) buildRequestBody(req *llm.ChatRequest, stream bool) ([]byte, error) {
	...
	// OrcaRouter normalizes reasoning across the upstream providers it fronts.
...
// plugin represents an OpenRouter-style web search plugin configuration.
type plugin struct {
	ID         string `json:"id"`
...
// reasoningRequest represents a unified reasoning configuration.
