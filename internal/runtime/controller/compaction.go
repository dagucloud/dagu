...
func (s *State) LatestPromptTokens() int {
	for i := len(s.messages) - 1; i >= 0; i-- {
		msg := s.messages[i]
		if msg.Role == exec.RoleAssistant && msg.Metadata != nil && msg.Metadata.PromptTokens > 0 {
...
func decisionReferences(messages []exec.LLMMessage) map[string]decisionReference {
	result := make(map[string]decisionReference)
	turn := 0
	for _, msg := range messages {
		if msg.Role != exec.RoleAssistant {
			continue
		}
		turn++
