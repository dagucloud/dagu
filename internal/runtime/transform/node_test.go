...
func TestNodeFieldsRoundTrip(t *testing.T) {
	...
	original := &exec.Node{
		...
		Repeated:               true,
		Error:                  "test error",
		SubRuns:                []exec.SubDAGRun{{DAGRunID: "sub-1", Params: "p1", ParallelItem: new("item-1")}},
		SubRunsRepeated:        []exec.SubDAGRun{{DAGRunID: "sub-2", Params: "p2", ParallelItem: new("item-2")}},
