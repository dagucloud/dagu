...
func TestSockAddr(t *testing.T) {
	...
	t.Run("Location", func(t *testing.T) {
		dag := &ir.DAG{Location: "testdata/testDag.yml"}
	...
	t.Run("BasicFunctionality", func(t *testing.T) {
		...
		require.Equal(t, addr, ir.SockAddr("mydag", "run123"))
	})

	t.Run("DifferentInputsProduceDifferentHashes", func(t *testing.T) {
		t.Parallel()

		addr1 := ir.SockAddr("dag1", "run1")
		addr2 := ir.SockAddr("dag1", "run2")
		addr3 := ir.SockAddr("dag2", "run1")
	...
	t.Run("SafeNameHandling", func(t *testing.T) {
		...
		addr := ir.SockAddr("my/dag\\with:special*chars", "run|with<>chars")
	...
	t.Run("MaxSocketLengthEnforcement", func(t *testing.T) {
		...
		addr := ir.SockAddr(strings.Repeat("a", 100), "run123")
		socketName := filepath.Base(addr)
	...
	t.Run("EdgeCaseTruncation", func(t *testing.T) {
		...
		socketName32 := filepath.Base(ir.SockAddr(name32, "run123"))
		socketName33 := filepath.Base(ir.SockAddr(name33, "run123"))
	...
	t.Run("DAGMethodsUseSockAddr", func(t *testing.T) {
		...
		dag1 := &ir.DAG{Name: "mydag", Location: "path/to/dag.yml"}
		require.Equal(t, ir.SockAddr("path/to/dag.yml", "run123"), dag1.SockAddr("run123"))
		require.NotEqual(t, dag1.SockAddr("run123"), dag1.SockAddr("run456"))

		// Without Location: uses Name and dagRunID
		dag2 := &ir.DAG{Name: "mydag"}
		require.Equal(t, ir.SockAddr("mydag", "run123"), dag2.SockAddr("run123"))
	})

	t.Run("SockAddrForSubDAGRun", func(t *testing.T) {
		t.Parallel()

		dag := &ir.DAG{Name: "parentdag", Location: "path/to/parent.yml"}
		subRunID := "child-run-456"

		// Uses DAG name (not location) with the sub run ID
		require.Equal(t, ir.SockAddr("parentdag", subRunID), dag.SockAddrForSubDAGRun(subRunID))
	})
}
