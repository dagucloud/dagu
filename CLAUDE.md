...
- **`ir/`** — Canonical normalized DAG and step representation shared by storage and execution. It owns configuration value objects, lifecycle enums, compatibility decoding, cloning, defaults, and intrinsic queries. Three DAG execution types: `graph` (default), `chain`, `controller` (LLM-driven step ordering).
