...
- **Domain contracts** — Ports live with their owning concepts: `dagrun` (run status, attempts, run stores), `dagstore` (DAG loading/storage), `queue`, `proc`, `dispatch` (distributed dispatch, worker and lease stores), `serviceregistry`, `incremental`, and `workspace`. Runtime condition results live in `dagrun`; `ir.Condition` remains definition-only.
