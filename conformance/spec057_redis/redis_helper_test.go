...
const (
	redisContainerPrefix = "dagu-conformance-redis"
	// redisImage is pinned by digest (a multi-arch manifest list) for a
...
// "stdout:" line at all, meaning the step wrote zero bytes to stdout.
...
// startRedisContainer starts a fresh Redis container -- under a name unique
...
// 6379/tcp to a Docker-assigned host port, waits for it to answer PING,
// and registers cleanup by container ID. It returns the host port Docker
// actually bound, read back after the container starts.
func startRedisContainer(t *testing.T, dockerClient *client.Client) int {
	t.Helper()

	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()

	containerName := uniqueName(t, redisContainerPrefix)
	redisPort := network.MustParsePort("6379/tcp")
	created, err := dockerClient.ContainerCreate(ctx, client.ContainerCreateOptions{
		Config: &container.Config{
			Image:        redisImage,
			ExposedPorts: network.PortSet{redisPort: {}},
		},
		HostConfig: &container.HostConfig{
			AutoRemove: true,
			PortBindings: network.PortMap{
				// Empty HostPort: let Docker assign a free host port itself.
				redisPort: {{HostIP: netip.MustParseAddr("127.0.0.1")}},
			},
		},
		Name: containerName,
	})
	...
	hostPort := resolvedHostPort(t, ctx, dockerClient, containerID, redisPort)
	waitForRedis(t, hostPort)
	return hostPort
}
...
func waitForRedis(t *testing.T, port int) {
	t.Helper()
	...
	t.Fatalf("redis never became reachable on 127.0.0.1:%d", port)
}

// setRedisKeyExternally sets a key directly against the Redis instance on
// port, bypassing dagu entirely -- used to simulate state dagu did not
// itself create, such as a lock already held by another process.
func setRedisKeyExternally(t *testing.T, port int, key, value string, ttl time.Duration) {
	t.Helper()
...
// redisKeyExists checks whether key exists in the Redis instance on port,
// bypassing dagu entirely.
func redisKeyExists(t *testing.T, port int, key string) bool {
	t.Helper()
