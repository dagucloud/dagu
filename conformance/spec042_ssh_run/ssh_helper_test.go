...
const (
	...
	sshContainerNamePrefix = "dagu-conformance-sshd"
)

// sshTestImage is content-addressed from sshdDockerfile's own text: a tag
// that only depends on a mutable global ("dagu-conformance-sshd:test")
// would let a stale image from a previous, different version of
// sshdDockerfile silently satisfy the "already built" check below. Editing
// the Dockerfile therefore always produces a different tag -- a genuine
// cache hit only happens when the content is actually unchanged.
var sshTestImage = "dagu-conformance-sshd:" + dockerfileTag(sshdDockerfile)
...
// claim that port first.
func startSSHDContainer(t *testing.T, dockerClient *client.Client) int {
	t.Helper()
