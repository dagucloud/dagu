...
| Run collection | `data.items` is an array. Each item has `name` string, `dagRunId` string, `uri` string, `status` number, and `statusLabel` string. `uri` is the canonical `dagu://runs/{name}/{dagRunId}` URI. A started item has `startedAt` string, and a finished item has `finishedAt` string. When another page is available, `data.nextCursor` is an opaque cursor string. |
| Run detail | `data.name` is the DAG name string. `data.dagRunId` is the DAG-run ID string. `data.uri` is the canonical `dagu://runs/{name}/{dagRunId}` URI. `data.logsUri` is the canonical `dagu://runs/{name}/{dagRunId}/logs` URI. `data.status` is a number. `data.statusLabel` is a string. A started run has `startedAt` string, and a finished run has `finishedAt` string. `data.steps` is an array with one entry per step in execution order; each entry has `name` string, `status` number, `statusLabel` string, and `logUri` string, and a failed step carries its `error` string when one was recorded. |
