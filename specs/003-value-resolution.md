...
| `steps[].worker_selector` | Value-resolved | Before the sub-DAG run request is created | Step worker selector keys and values resolve Dagu-owned references. Parallel `dag.run` children resolve with the item context available as `${ITEM}`. `dag.enqueue` selectors must be literal because enqueue persists a durable execution snapshot. |
