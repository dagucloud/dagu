...
- Sub-DAG runs inherit the already-selected parent run profile. Runtime-managed sub-DAG paths must not read control-plane DAG settings; queued sub-DAG runs persist the inherited profile plus parent/root hierarchy metadata at enqueue time.
