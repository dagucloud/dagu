...
Automate a website in a local Chrome with natural-language operations. The model comes from the DAG-level `llm` block, or `with.llm`, which replaces it entirely. Chrome or Chromium must be installed on the host that runs the step (`browser.executable` or `CHROME_PATH` selects it).
...
- Each `do` item sets exactly one of `goto`, `act`, `extract`, `expect`, `wait` (`selector` or `duration`), `screenshot`, or `ask`, plus optional `when` (skip unless true) and `timeout`.
- Put secrets in `variables` and reference them as `%name%`; the browser gets the value, the model only the name. An instruction containing a secret value fails the step. Do not write `${SECRET}` inside an instruction.
...
- `expect` fails the step when the statement is false.
- Successful `act` operations are replayed on later runs without a model call; set `cache: false` on the step or on one `act` to always ask the model.
- `ask: {prompt, as}` puts the step in Waiting until someone answers in the Web UI; the answer becomes `%<as>%`. Use it for short-lived codes; answers are stored in run history.
- Screenshots (on failure and at the end by default) and downloads are stored under `browser/<step id>/` in the run artifacts.
