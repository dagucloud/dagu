...
- Hosts add browser flags for every browser step with `browser.args` in the Dagu config, or `DAGU_BROWSER_ARGS` as a comma-separated list, for example `--no-sandbox,--disable-dev-shm-usage` in a container where the browser sandbox is unavailable. DAGs cannot set them.
