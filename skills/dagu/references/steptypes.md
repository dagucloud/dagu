...
Automate a website in a local Chrome with natural-language operations. The model comes from the DAG-level `llm` block, or `with.llm`, which replaces it entirely; use a model that follows tool-call schemas reliably. Chrome or Chromium must be installed on the host that runs the step (`browser.executable` or `CHROME_PATH` selects it). Of the container images, only `ghcr.io/dagucloud/dagu:dev` includes Chromium, on amd64 and arm64; run it with `--security-opt seccomp=deploy/docker/seccomp-chromium.json` so the browser sandbox can start, and `--shm-size=1g`. The profile applies to every process in the container and lets DAG steps call `clone`, `setns`, and `unshare` without argument filters, so use it only when the container's DAG steps are trusted.
...
- Hosts add browser flags for every browser step with `browser.args` in the Dagu config, or `DAGU_BROWSER_ARGS` as a comma-separated list. `--no-sandbox` turns off the browser sandbox where it cannot start; a compromised page then runs with the Dagu process's permissions, so prefer the seccomp profile. DAGs cannot set these flags.
