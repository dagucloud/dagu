...
- `expect` and `when` take a statement the model judges, or a fixed check `{text}`, `{selector}`, or `{url}` that reads the page without a model call. Prefer fixed checks for monitoring; they give the same result on every run.
...
- `allowed_domains` applies to every request the page makes. `example.com` matches only that host; `*.example.com` matches its subdomains. The step fails when the page leaves the list.
...
- Downloads are saved under `browser/<step id>/downloads/` in the run artifacts and awaited before the step ends. Screenshots are saved on failure by default (`screenshots: final` also keeps one of a successful end). Artifacts are not masked.
